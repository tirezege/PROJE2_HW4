...
public:
    ...
    virtual ~Effect(){
        ...
        glDeleteTextures(1, &ColorCorrMap);
    ...
    virtual void draw(){
		...
		glBindTexture(GL_TEXTURE_1D, ColorCorrMap);
...
private:
    ...
    void initShader(const char* fragmentShaderFile){
		ColorCorrMap = SOIL_load_OGL_texture
		(
		"efekt.bmp",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		...
		UniformTexture = glGetUniformLocation(programID, "ColorCorrMap");
        if (UniformTexture == -1) {
            fprintf(stderr, "Could not bind attribute %s\n", "ColorCorrMap");
        }
